...
import requests
from llm.glm4_9b_no_proxy import generate_response
from llm.secret_file import api_keys
from tqdm import tqdm
from utils import metrics

data_types = ['valid', 'test']# 
test_num = 'consist_wo_comment_0'# wo_comment_
...
def get_result(data, api_key, store_path):
    ...
    results = []
    responses = []
    responses = generate_response(llm_type, sys_prompt=sys_prompt, user_prompt=user_prompt, n=5, temperature=0.7)
    for response in responses[-1]['content']:
        response = int(re.findall(r'\d+', response)[-1])
        results.append(response)
    save_file['log'] = responses
    save_file['result'] = results
...
def threshold_set(threshold, store_path, data_path):
    ...
    for i in os.listdir(store_path):
        with open(f"{store_path}/{i}", 'r', encoding='utf-8-sig') as file:  
            data = json.load(file)  
            response = np.mean(data['result'])
            # response = stats.mode(data['result'])[0]
...
if __name__ == "__main__":
    for data_type in data_types:
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:# len(api_keys)
            futures = []
