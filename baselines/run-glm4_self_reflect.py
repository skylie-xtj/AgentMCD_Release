...
test_num = 'reflect_1'# wo_comment_
...
def get_result(data, api_key, store_path):
    ...
    user_prompt = f"""
以下是其他人根据视频信息做出的判断争议的依据和争议评分:
{response[-1]['content']}
你能再检查一下你的答案是否正确吗？请做出你判断争议的依据，并输出争议评分（0-9）。
""".removeprefix("\n")
    response = generate_response(llm_type, history=response, sys_prompt=None, user_prompt=user_prompt, api_key=api_key) 
