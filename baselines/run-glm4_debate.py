...
def suggest_prompt(response):
    user_prompt = f"""以下是其他代理对问题的解决方案：
{response}
如果使用其他代理的意见作为附加建议，你能给出一个更新的回答吗?
请做出你判断争议的依据，并输出争议评分（0-9）。
""".removeprefix("\n")
    return user_prompt
...
def get_result(data, api_key, store_path):
    ...
    response0 = generate_response(llm_type, history=[], temperature=0.7, sys_prompt=sys_prompt, user_prompt=user_prompt, api_key=api_key)
    response1 = generate_response(llm_type, history=[], temperature=0.7, sys_prompt=sys_prompt, user_prompt=user_prompt, api_key=api_key)
    result0 = int(re.findall(r'\d+', response0[-1]['content'].split('\n争议评分')[-1])[0])
    result1 = int(re.findall(r'\d+', response1[-1]['content'].split('\n争议评分')[-1])[0])
    round = 0
    while result0 != result1:
        if round == 4: break
        round += 1
        user_prompt = suggest_prompt(response0[-1]['content'])
        response1 = generate_response(llm_type, history=response1, sys_prompt=None, user_prompt=user_prompt, api_key=api_key)
        user_prompt = suggest_prompt(response1[-1]['content'])
        response0 = generate_response(llm_type, history=response0, sys_prompt=None, user_prompt=user_prompt, api_key=api_key)
        result0 = int(re.findall(r'\d+', response0[-1]['content'].split('\n争议评分')[-1])[0])
        result1 = int(re.findall(r'\d+', response1[-1]['content'].split('\n争议评分')[-1])[0])
    save_file['agent0'] = response0
    save_file['agent1'] = response1
    # response = int(re.findall(r'\d+', response[-1]['content'])[-1])
    save_file['result'] = int(np.mean([result0, result1]))
